...
execute "authenticate" do
  command "curl -v -k -c /var/tmp/jazzCookies.txt -u #{node['clm']['admin_userid']}:#{node['clm']['admin_password']} https://#{node['ec2']['public_hostname']}:9443/ccm/secure/authenticated/identity"
  action :run
end

execute "security check" do
  command "curl -v -k -b /var/tmp/jazzCookies.txt -c /var/tmp/jazzCookies.txt -d j_username=#{node['clm']['admin_userid']} -d j_password=#{node['clm']['admin_password']} https://#{node['ec2']['public_hostname']}:9443/ccm/authenticated/j_security_check"
  action :run
end

execute "RTC requestRest" do
  command "curl -v -k -b /var/tmp/jazzCookies.txt -c /var/tmp/jazzCookies -u #{node['clm']['admin_userid']}:#{node['clm']['admin_password']} https://#{node['ec2']['public_hostname']}:9443/ccm/admin/cmd/requestReset"
  action :run
end

execute "RQM requestRest" do
  command "curl -v -k -b /var/tmp/jazzCookies.txt -c /var/tmp/jazzCookies -u li#{node['clm']['admin_userid']}:#{node['clm']['admin_password']} https://#{node['ec2']['public_hostname']}:9443/qm/admin/cmd/requestReset"
  action :run
end
